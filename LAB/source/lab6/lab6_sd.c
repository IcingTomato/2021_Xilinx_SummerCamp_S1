...
#define LAB6_BITFILE_LEN  0x03dbb00 // BIN formatted BITfile length
#define LAB2BITFILE_LEN  0x03dbb00
#define LAB6_ELFBIN_ADDR  0x00200000 // BIN formatted ELF address
#define LAB2_ELFBIN_ADDR  0x00600000
#define LAB6_ELFBINFILE_LEN  0x00008008 // BIN formatted ELF length
#define LAB2_ELFBINFILE_LEN  0x00008008
#define LAB6_ELF_EXEC_ADDR  0x002005b8 // ELF main() entry point
#define LAB2_ELF_EXEC_ADDR  0x006005b8
	...
	while(Exit != 1) {
		...
		switch (OptionNext) {
			...
			case '1':
				...
				Xil_DCacheDisable();
				xil_printf("Loading lab6 BIT file\n\r");
				SD_LoadBitFile("lab6.bin", BITFILE_ADDR, (LAB6_BITFILE_LEN << 2));
				...
				Status = XDcfg_TransferBitfile(XDcfg_0, BITFILE_ADDR, LAB6_BITFILE_LEN);
				if (Status != XST_SUCCESS) {
					xil_printf("Error : FPGA configuration failed!\n\r");
					exit(EXIT_FAILURE);
				}
				xil_printf("Lab6.bin loaded!, executing its application.\n\r");
				Xil_Out32(0xF8000008,0x0000DF0D); // Unlock devcfg.SLCR
				Xil_Out32(0xF8000900, 0xF); // turn-ON the level shifter
				Xil_Out32(0xF8000004,0x767B); // Lock devcfg.SLCR
				SD_ElfLoad("lab6elf.bin", LAB6_ELFBIN_ADDR, LAB6_ELFBINFILE_LEN);
				load_elf(LAB6_ELF_EXEC_ADDR);
				break;
			case '2':
				// Flush and disable Data Cache
				Xil_DCacheDisable();
				xil_printf("Loading lab2 BIT file\n\r");
				SD_LoadBitFile("lab2.bin", BITFILE_ADDR, (LAB2BITFILE_LEN << 2));
				// Invalidate and enable Data Cache
				Xil_DCacheEnable();
				Xil_Out32(0xF8000008,0x0000DF0D); // Unlock devcfg.SLCR
				Xil_Out32(0xF8000900,0xA); // turn-off the level shifter
				Xil_Out32(0xF8000004,0x767B); // Lock devcfg.SLCR
				Status = XDcfg_TransferBitfile(XDcfg_0, BITFILE_ADDR, LAB2BITFILE_LEN);
				if (Status != XST_SUCCESS) {
					xil_printf("Error : FPGA configuration failed!\n\r");
					exit(EXIT_FAILURE);
				}
				xil_printf("Lab2.bin loaded!, executing its application.\n\r");
				Xil_Out32(0xF8000008,0x0000DF0D); // Unlock devcfg.SLCR
				Xil_Out32(0xF8000900, 0xF); // turn-ON the level shifter
				Xil_Out32(0xF8000004,0x767B); // Lock devcfg.SLCR
